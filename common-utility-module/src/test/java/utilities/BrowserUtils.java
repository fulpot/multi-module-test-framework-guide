...
public class BrowserUtils {
    ...
    // To read a QR code by taking the screenshot of the web element and decoding
    public static String qrDecode(WebElement webElement) {
        ((JavascriptExecutor) Driver.getDriver()).executeScript("arguments[0].scrollIntoView({block:'center'});", webElement);
        byte[] png = webElement.getScreenshotAs(OutputType.BYTES);
        Result result = null;
        try {
            BufferedImage image = ImageIO.read(new ByteArrayInputStream(png));
            if (image == null) throw new IllegalArgumentException("The image could not be read");
